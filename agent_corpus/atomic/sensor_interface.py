    ...
    def run(self):
        ...
        while self._run_ps:
            if self._callback is not None:
                ...
                if round(current_time - latest_time, 6) >= (1 / self._reading_frequency) \
...
class OpenDriveMapReader(BaseReader):
    ...
    def __init__(self, vehicle, opendrive_data, reading_frequency=1.0):
        # self._opendrive_data = DataProvider.get_map().to_opendrive()
        self._opendrive_data = opendrive_data
...
class SensorInterface(object):
    ...
    def get_data(self, frame):
        ...
        try:
            ...
            logger.error(f"Timeout while waiting for sensor data: ")
            logger.error(f"Sensor data: {self._sensors_objects.keys()}")
            logger.error(f"Sensor data received: {data_dict.keys()}")
