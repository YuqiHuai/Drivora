...
class AgentWrapper(object):
    ...
    def __init__(self, agent, ctn_operator: CtnSimOperator):
        """
    ...
    def setup_sensors(self):
        ...
        for sensor_spec in self._agent.sensors():
            ...
            if sensor_spec['type'].startswith('sensor.opendrive_map'):
                ...
                sensor = OpenDriveMapReader(vehicle, self.ctn_operator.get_map().to_opendrive(), sensor_spec['reading_frequency'])
            elif sensor_spec['type'].startswith('sensor.speedometer'):
                logger.info("Setting up Speedometer sensor")
                delta_time = self.ctn_operator.get_world().get_settings().fixed_delta_seconds
                logger.info(f"World delta time: {delta_time}")
