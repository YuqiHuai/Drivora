    ...
    def compute_returns_and_advantage(self, last_value: th.Tensor, dones: np.ndarray) -> None:
        ...
        for step in reversed(range(self.buffer_size)):
            if step == self.buffer_size - 1:
                ...
                # self.returns[step] = self.reward[step] + self.gamma * last_value * next_non_terminal
            else:
                next_non_terminal = 1.0 - self.dones[step + 1]
                next_value = self.values[step + 1]
                # spinning up return calculation
                # self.returns[step] = self.reward[step] + self.gamma * self.returns[step+1] * next_non_terminal
