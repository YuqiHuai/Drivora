...
from agent_base import ERDOSBaseAgent, \
    process_visualization_events, read_control_command, using_lidar, using_perfect_component
...
class AgentSystem(ERDOSBaseAgent):
    ...
    def setup(self, path_to_conf_file):
        super(AgentSystem, self).setup(path_to_conf_file)
    ...
    def run_step(self, input_data, timestamp):
        ...
        imu_data = None
        gnss_data = None
        self.logger.debug(f"@{timestamp}: Received input data: {input_data.keys()}")
        for key, val in input_data.items():
            ...
            if key in self._camera_streams:
                ...
                if not self._open_drive_stream.is_closed():
                    ...
                    self.logger.debug(
                        f"@{timestamp}: Sending OpenDrive data on stream.")
        ...
        #     # if we use the sensors-only track, we do not have access to the
        #     # We do not have access to the open drive map. Send top watermark.
        #     self.logger.debug(f"@{timestamp}: Sending top watermark on open drive stream.")
        #     self._open_drive_stream.send(
        #         erdos.WatermarkMessage(erdos.Timestamp(is_top=True)))
        ...
        return command, {} # TODO: add logs
    
        # NOTE: currently, we ignore this
